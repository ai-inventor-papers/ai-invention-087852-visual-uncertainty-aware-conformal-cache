...
# Papers without ArXiv IDs - we'll search for them
# These are well-known papers, so we can construct BibTeX from known info
KNOWN_PAPERS = [
    {
        "key": "Yang2023",
    ...
    },
    {
        ...
        "author": "Nikoletta Iliakopoulou and Stefan Stojkovic and Christos Alverti and Yucheng Xu and Armin Franke and Jos{\'e} Torrellas",
        "year": "2024",
...
def main():
    ...
    for paper in ARXIV_PAPERS:
        ...
        if bibtex:
            ...
            # Fallback: construct from known info
            key = f"{paper['author']}{paper['year']}"
            lines = [f"@misc{{{key}{{",
                     f"  title = {{{paper['title']}}},",
                     f"  author = {{{paper['author']}} et al.},",
                     f"  year = {{{paper['year']}}},",
                     f"  eprint = {{{arxiv_id}}},",
                     f"  archivePrefix = {{arXiv}},"]
            if paper.get("venue"):
                lines.append(f"  note = {{{paper['venue']}}},")
            lines.append("}")
            entries.append("\n".join(lines))
